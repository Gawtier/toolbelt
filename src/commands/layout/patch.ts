...
async function readInput(filePath: string | undefined): Promise<PatchInput> {
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(
      'Invalid patch input: expected a JSON object with domain keys (e.g. {"layout": [...], "workflows": [...]}).',
    );
  }
...
export function toPlannedOps(domain: LayoutDomain, rawOps: RawOp[]): PlannedOp[] {
...
export function buildAllOps(input: PatchInput): PlannedOp[] {
  return LAYOUT_DOMAINS.flatMap(domain => {
...
/**
 * `forest layout patch` — send a JSON-Patch array directly to the environment,
 * without pulling the current layout first. Designed for the forest-layout skill's
 * intent-based mode: the caller decides add vs replace, the command validates +
 * forwards.
 *
 * Input format (file or stdin):
 * {
 *   "layout":    [{ "op": "add", "path": "/...", "value": {...} }],
 *   "workflows": [{ "op": "replace", "path": "/...", "value": "..." }],
 *   "folders":   [...]
 * }
 */
export default class LayoutPatchCommand extends AbstractAuthenticatedCommand {
  ...
  static override description =
    'Apply a JSON-Patch array directly to an environment (no pull required).';
    ...
    for (const domain of LAYOUT_DOMAINS) {
      ...
      try {
        ...
        if (error instanceof LayoutApiError && error.status !== 401) {
          // Scope error to this domain so path correlation in explainApiError is accurate.
